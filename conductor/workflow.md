...
uv run ruff format . && uv run ruff check . --fix && uv run ty . && uv run pytest
```
...
1. All code is implemented to specification, with relevant tests verifying and locking in the functionality.
2. Unit tests are written (using a TDD approach) and passing.
